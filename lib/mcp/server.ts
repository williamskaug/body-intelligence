...
export function buildMcpServer(ctx: McpContext): McpServer {
  ...
  server.registerTool(
    "get_setup_guide",
    {
      ...
      description:
        "READ THIS FIRST when connecting to Body Intelligence. Returns the usage guide: data model, health-event severity (5=worst), the standard memory file paths and their purposes, idempotency rules, and the intended reasoning rhythm. Call this once at the start of a session before reasoning about the user's data.",
  ...
  server.registerTool(
    "log_daily",
    {
      ...
      description:
        "Upsert the daily entry for (user, date). Upsert merge semantics: only the fields you pass are touched; omitted fields keep their existing values. The schema does not accept null to clear — to wipe a field, use delete_daily_entry and re-log. Accepts Garmin/Whoop/Oura-derived vitals including skin_temp_deviation_c, sleep_score, stress_score, the Body Battery scalars (morning/high/low/charged/drained), training_readiness_score, and training_status. Vendor composite SCALARS live in columns so they can be trended/correlated; the factor breakdowns, hourly curves, and vendor labels stay in daily/YYYY-MM-DD.md documents. training_readiness_score is a captured observation — NOT BI's readiness gate, which stays the agent's authored call in derived_daily.",
  ...
  server.registerTool(
    "log_health_event",
    {
      ...
      description:
        "Insert a new injury, illness, or symptom. severity is 1-5 with 5 = most severe. Resolve later via update_health_event(id, resolved_date=...) — don't delete, the history matters for the next injury that looks similar.",
  ...
  server.registerTool(
    "get_recent",
    {
      ...
      description:
        "Bundle workouts / daily entries / health events / derived rows from the last N days. 'derived' is the agent-computed layer (readiness gate, z-scores, sleep debt). Unresolved health events older than the window still come back. Use kinds= to subset.",
  ...
  server.registerTool(
    "search_everything",
    {
      ...
      description:
        "Text search over all entity prose fields (workout notes, daily reflections, health-event notes) plus document content. Sorted by updated_at desc.",
  ...
  server.registerTool(
    "get_streak",
    {
      ...
      description:
        "Return current_streak_days, best_streak_days (over the past 365 days), and a 30-element boolean sparkline. Kinds: 'daily_entry' (any row in daily_entries that day), 'workout' (any workout that day). Use to power streak surfaces and to detect streak-loss events for the attention panel.",
  ...
  server.registerTool(
    "get_calendar",
    {
      ...
      description:
        "Return one row per day in the given (year, month) with workout_count, workout_total_min, workout_types, has_daily_entry, and active_health_event_count. Cheaper than fetching all rows and rolling up client-side when you only need the calendar grid.",
