...
export function buildMcpServer(ctx: McpContext): McpServer {
  ...
  server.registerTool(
    "log_daily",
    {
      ...
      description:
        "Upsert the daily entry for (user, date). Upsert merge semantics: only the fields you pass are touched; omitted fields keep their existing values. The schema does not accept null to clear — to wipe a field, use delete_daily_entry and re-log. Accepts Garmin/Whoop/Oura-derived vitals including skin_temp_deviation_c, sleep_score, stress_score, the Body Battery scalars (morning/high/low/charged/drained), training_readiness_score, training_status, weight_kg, and movement totals. Vendor composite SCALARS live in columns so they can be trended/correlated; the factor breakdowns, hourly curves, and vendor labels stay in daily/YYYY-MM-DD.md documents. training_readiness_score is a captured observation — NOT BI's readiness gate, which stays the agent's authored call in derived_daily.",
  ...
  server.registerTool(
    "get_recent",
    {
      ...
      description:
        "Bundle workouts / daily entries / health events / derived rows / capacity snapshots from the last N days. 'derived' is the agent-computed layer (readiness gate, z-scores, sleep debt). Unresolved health events older than the window still come back. Use kinds= to subset.",
