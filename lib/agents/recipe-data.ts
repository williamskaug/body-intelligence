...
**GOALS.md** — ask for: 1-3 PRs/PBs they care about and any upcoming races (name, date, tier, goal). Help them format race blocks correctly — the dawn agent parses these for days_to_race. Don't pressure them to fill in races they don't have; an empty race section is fine. fs_write.
...
**5. Confirm.** Send the user one short line summarising what was synced. No editorialising on the numbers — that's the dawn agent's job.
...
- get_correlation_matrix(metrics=[hrv_ms, rhr_bpm, sleep_h, derived_sleep_debt_7d_min, derived_acute_load_7d, stress_score, weight_kg], window_days=90) → the relationships that are real for THIS athlete.
