...
> **Solo-user trim (migration `drop_meals_and_unused_daily_columns`):**
> - Dropped the `meals` table (10 historical rows from May 2026; destruction accepted).
> - Dropped unused `daily_entries` columns: subjective 1–5 scales, empty body-comp/vitals, and `meal_notes`. Kept sleep/HRV/RHR/weight/movement/recovery scalars and sleep/wellness notes.
...
| `skin_temp_deviation_c` | `numeric(4,2)` | overnight skin/wrist temp Δ vs personal baseline, °C |
| `sleep_score` | `smallint` | vendor 0–100 last-night sleep score |
| `stress_score` | `smallint` | vendor 0–100 daily average stress |
| `body_battery_morning` | `smallint` | 0–100 on waking |
| `body_battery_high` | `smallint` | 0–100 day max |
| `body_battery_low` | `smallint` | 0–100 day min |
| `body_battery_charged` | `smallint` | 0–100 charged |
| `body_battery_drained` | `smallint` | 0–100 drained |
| `training_readiness_score` | `smallint` | vendor 0–100 morning readiness — a captured observation, NOT BI's gate |
| `training_status` | `text` | free lowercased vendor string (no CHECK) |
...
| `sleep_notes` | `text` | qualitative only |
| `wellness_notes` | `text` | qualitative only |
| `created_at` | `timestamptz default now()` | |
| `updated_at` | `timestamptz default now()` | |

Constraints: `unique (user_id, date)`. Check constraints on 0–100 vendor scores (sleep_score, stress_score, Body Battery scalars, training_readiness_score).
...
- `daily_entries` new columns: `stress_score`, `body_battery_morning/high/low/charged/drained`,
  ...
  CHECK**). Body-comp/vitals added here (`muscle_mass_kg`, `bone_mass_kg`,
  `body_water_pct`, `bp_systolic_mmhg`, `bp_diastolic_mmhg`, `hydration_ml`)
  were later dropped in the solo-user trim. Range CHECKs in the `daily_*_range` style.
