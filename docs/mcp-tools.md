...
- **Bulk writes:** `bulk_log_workouts`, `bulk_log_daily`, `bulk_log_capacity` (up to 500/call, returns `{ inserted, updated, errors[] }`)
...
```ts
{
  ...
  skin_temp_deviation_c?: number; // overnight skin/wrist temp Δ, °C
  ...
  stress_score?: number;             // 0-100 daily average
  ...
  training_readiness_score?: number; // 0-100; NOT BI's readiness gate
  training_status?: string;          // free lowercased vendor string
  // Free-text (qualitative only)
  sleep_notes?: string;
...
- Subjective 1–5 wellness scales, unused body-comp/vitals, and `meal_notes` were dropped in the solo-user trim. Qualitative flags go in `wellness_notes`.
...
{
  days: number;                                   // 1-90
  kinds?: Array<"workouts" | "daily" | "health_events" | "derived" | "capacity">; // default: all five
}
```
...
Implementation: union four separate full-text queries (workouts, daily_entries, health_events, documents) and merge by `updated_at` desc with limit applied at the end.
...
- `log_daily` / `bulk_log_daily` gain the kept `daily_entries` columns (stress,
  Body Battery scalars, training readiness/status). Unused body-comp/vitals
  and subjective 1–5 scales were later dropped in the solo-user trim.
